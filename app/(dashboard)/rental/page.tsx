  ...
  const columns: import("@tanstack/react-table").ColumnDef<RentalItem, any>[] = [
    ...
    },
    {
      ...
      header: ({ column }: { column: Column<RentalItem, unknown> }) => {
        return (
          <Button variant="ghost" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>Item Name</Button>
        )
      },
      cell: ({ row }: { row: Row<RentalItem> }) => (
        <Link
    ...
    },
    {
      ...
      header: "Price/Day",
      cell: ({ row }) => (
        <span className="font-semibold text-blue-700 whitespace-nowrap">${row.getValue("pricePerDay")}</span>
      ),
    },
    {
      ...
      header: "Location",
      cell: ({ row }) => (
        <span className="text-xs text-gray-500 max-w-[120px] block truncate">{row.getValue("location")}</span>
      ),
    },
    {
      accessorKey: "status",
      ...
      cell: ({ row }: { row: Row<RentalItem> }) => {
        ...
        return (
          <Badge
            ...
            className={status === "available" ? "bg-green-100 text-green-700 hover:bg-green-200" : ""}
          >
            {String(status)}
          </Badge>
        )
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuItem asChild>
                <Link href={`/rental/${item.id}/edit`} className="cursor-pointer">
    ...
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage all rental items in the system.</p>
        </div>
        <Button
          asChild
