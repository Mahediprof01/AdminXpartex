...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  const product = getProduct((actualParams as { id: string }).id)
    ...
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight truncate">{product.name}</h1>
                ...
                <div>
                  ...
                  <p
                    className={`text-sm font-semibold ${
                      product.stock === 0 ? "text-red-500" : product.stock < 50 ? "text-orange-500" : "text-green-600"
                    }`}
                  >
                    {product.stock} units
                  </p>
