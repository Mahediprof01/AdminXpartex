...
import { MoreHorizontal, Plus, Eye, Edit, Trash, Building2 } from "lucide-react"
import {
	DropdownMenu,
	DropdownMenuContent,
	DropdownMenuItem,
		...
		const columns: import("@tanstack/react-table").ColumnDef<Vendor, any>[] = [
			...
			},
			{
				...
				header: ({ column }: { column: Column<Vendor, unknown> }) => {
					return (
						<Button variant="ghost" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>Supplier Name</Button>
					)
				},
				cell: ({ row }: { row: Row<Vendor> }) => (
					<Link
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="end" className="w-48">
								...
								<DropdownMenuItem asChild>
									<Link href={`/suppliers-business/${vendor.id}`} className="cursor-pointer">
										<Eye className="mr-2 h-4 w-4 text-blue-500" />
										View Details
									</Link>
								</DropdownMenuItem>
								<DropdownMenuItem asChild>
									<Link href={`/suppliers-business/${vendor.id}/edit`} className="cursor-pointer">
